...
Model 1  
* x = health factor variables, median age, segregation score
* y = number of covid cases in 2021

```{r}
m1 = comb %>% 
  filter(year == '2021') %>% 
  lm(n_cases ~ health_beh_r + clinical_care_r + env_r + ses_r + medage, data = .)
...
```{r}
m2 = comb %>% 
  filter(year == '2021', county != "New York") %>% 
  lm(n_deaths ~ health_beh_r + clinical_care_r + env_r + ses_r + medage, data = .)
...
m3 = comb %>% 
  filter(year == '2021') %>% 
  lm(vax_dose1 ~ health_beh_r + clinical_care_r + env_r + ses_r + medage, data = .,)
...
m4 = comb %>% 
  filter(year == '2021') %>% 
  glm(vax_maj ~ segregation + medage + ses_r, data = .)
...
m5 = comb %>% 
  filter(year == '2021', county != "New York") %>% 
  lm(n_deaths ~ segregation_score + median_income + medage, data = .)
