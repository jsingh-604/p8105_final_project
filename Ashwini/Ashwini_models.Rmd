...
## NY County Land Area
```{r}
area = read_excel('grace/pop_density.xlsx') %>% 
  janitor::clean_names() %>% 
  rename(area = value)
```
...
testmod_small = 
  new_comb %>%
  filter(year == '2021') %>%
  mutate(county_size = if_else(tpop < 100000, "Small",
                       if_else(100000 <= tpop & tpop < 1000000, "Medium",
                       if_else(1000000 <= tpop, "Large", NA_character_)))) %>%
  mutate(county_size = fct_relevel(county_size, "Small")) %>%
